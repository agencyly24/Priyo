...
export const ProfileDetail: React.FC<ProfileDetailProps> = ({ profile, onBack, onStartChat }) => {
  ...
  // Combine the main profile image with the gallery for the carousel
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white p-4 md:p-10 animate-in fade-in duration-500 relative">
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-16">
        ...
        {/* Left: Interactive Portrait Carousel */}
          ...
          <div className="relative group">
            ...
            <div className="relative rounded-[3rem] overflow-hidden shadow-2xl border border-white/10 aspect-[3.5/5] bg-black">
              ...
              <div className="absolute inset-0 flex items-center justify-between px-4 pointer-events-none">
                <button 
                  onClick={handlePrev}
                  className="p-4 rounded-full bg-black/40 backdrop-blur-md text-white border border-white/10 pointer-events-auto hover:bg-pink-600/60 transition-all active:scale-90 shadow-xl"
                >
              ...
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2">
                {allMedia.map((_, idx) => (
                  <div 
                    key={idx}
                    className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentMediaIndex ? 'w-8 bg-pink-500' : 'w-2 bg-white/30'}`}
                  />
                ))}
              </div>
        ...
        <div className="w-full lg:w-7/12 flex flex-col py-4">
          ...
          {/* Premium Tab Bar */}
          <div className="flex bg-white/5 border border-white/10 rounded-3xl p-1.5 mb-10">
            {['appearance', 'character', 'gallery'].map((tab) => (
              <button 
                key={tab}
                onClick={() => setActiveTab(tab as any)}
                className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all duration-300 ${
                  activeTab === tab 
                  ? 'bg-gradient-to-r from-pink-600 to-rose-500 text-white shadow-xl shadow-pink-600/20' 
          ...
          <div className="flex-1 min-h-[400px]">
            {activeTab === 'appearance' && (
              <div className="grid grid-cols-2 gap-5 animate-in fade-in slide-in-from-right-4 duration-500">
                <AttributeCard label="Ethnicity" value={profile.appearance.ethnicity} />
                <AttributeCard label="Eye Color" value={profile.appearance.eyeColor} />
                <AttributeCard label="Body Type" value={profile.appearance.bodyType} />
                <AttributeCard label="Breast Size" value={profile.appearance.breastSize} />
                <AttributeCard label="Hair Style" value={profile.appearance.hairStyle} />
                <AttributeCard label="Hair Color" value={profile.appearance.hairColor} />
                <AttributeCard label="Outfit" value={profile.appearance.outfit} />
                <AttributeCard label="Current Vibe" value="Feeling Romantic" />
              </div>
            )}

            {activeTab === 'character' && (
              <div className="grid grid-cols-2 gap-5 animate-in fade-in slide-in-from-right-4 duration-500">
                <AttributeCard label="Personality" value={profile.personality} />
                <AttributeCard label="Occupation" value={profile.character.occupation} />
                <div className="col-span-2">
                   <AttributeCard label="Interest & Kinks" value={profile.character.kinks.join(', ')} />
                </div>
                <div className="col-span-2 bg-white/5 border border-white/10 p-6 rounded-[2rem] flex items-center justify-between">
                  <div>
                    <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Voice Engine</span>
                  ...
                  <button className="px-6 py-3 glass rounded-xl text-xs font-black uppercase tracking-widest text-pink-500 hover:bg-pink-500 hover:text-white transition-all">Preview Voice</button>
                </div>
              </div>
            )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-5 animate-in fade-in slide-in-from-right-4 duration-500">
                {allMedia.map((item, idx) => (
                  <div 
                    ...
                    className={`group relative aspect-[3/4] rounded-[2rem] overflow-hidden cursor-pointer border shadow-lg active:scale-95 transition-all ${idx === currentMediaIndex ? 'border-pink-500 ring-4 ring-pink-500/20' : 'border-white/5'}`}
                  >
                    {item.type === 'image' ? (
                      <img 
                        src={item.url} 
          ...
          <button 
            onClick={onStartChat}
            className="group w-full h-24 mt-12 relative overflow-hidden bg-gradient-to-r from-[#ff4d6d] to-[#ff758c] rounded-[2.5rem] font-black text-2xl uppercase tracking-[0.2em] shadow-[0_25px_60px_-15px_rgba(255,77,109,0.5)] active:scale-[0.98] transition-all flex items-center justify-center gap-4"
          >
